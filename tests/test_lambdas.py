...
class TestCache(BaseTestCase):
    ...
    def test_docscore_lambda(self):
        p = pt.lambdas.doc_score(lambda doc_row: len(doc_row["text"]))
    ...
    def test_docfeatures_lambda(self):
        import numpy as np
        p = pt.lambdas.doc_features(lambda doc_row: np.array([0,1]) )
        testDF = pd.DataFrame([["q1", "hello", "d1", "aa"]], columns=["qid", "query", "docno", "text"])
        rtr = p(testDF)
